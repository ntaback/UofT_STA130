...
(b) Assume you conduct a hypothesis test using simulation and get the following empirical distribution for values of the test statistic $\hat{p}$, assuming the null hypothesis is true. For simplicity, this distribution only shows the results of 500 simulations. There are 500 dots on the plot, one for each simulation (note that in practice, 500 simulations is not sufficient).  What does each dot on the plot represent?
...
In August 2017, researchers at Dalhousie university conducted a [poll]((https://cdn.dal.ca/content/dam/dalhousie/pdf/management/News/Preliminary%20results%20cannibis-infused%20foods%20EN.pdf)) to learn more about attitudes towards the legalization of marijuana across Canada. They found that 68% of respondants supported the legalization of recreational marijuana, based on a sample of 1,087 adults who had been living in Canada for at least 12 months.  

Suppose a headline reads "New survey reports  support for legalization of recreational marijuana in Canada".  

(a) State the null hypothesis and alternative hypothesis to test if the claim in the headline is supported by the survey data.
...
Which of the estimated p-values do you think is the best estimate of the true p-value?  What happens to the runtime as the number of simulations increases? 
...
(b) Write a function to test this hypothesis which takes the following arguments:
    - `prop_cloning`: the proportion of adults  who believe cloning should be allowed
...
(c) For each of the statements below, use your function from (b) to determine which of the following headlines are compatible with the data. In each case, interpret the p-value and draw a conclusion.
