...
flight_tailnum %>% inner_join(plane_year) %>% summarise(min_year = min(plane_year))

# Calculate the number of planes that flew from NYC that are in planes table
flights %>% inner_join(planes, by = "tailnum") %>% summarize(n=n_distinct(tailnum))
...
planes2 %>% ggplot(aes(x = year, y = n)) + geom_col(aes(fill = manufacturer_cat))
...
weather %>% filter(year == 2013 & month ==7) %>% ggplot(aes(temp)) + geom_histogram(binwidth = 1)

weather %>% filter(year == 2013 & month ==7) %>% ggplot(aes(x ="",temp)) + geom_boxplot() + labs(x = "")

weather %>% ggplot(aes(x ="",wind_speed)) + geom_boxplot() + labs(x = "")

weather %>% ggplot(aes(x = dewp , y = humid)) + geom_point() 

 
weather %>% ggplot(aes(x = visib, y = precip)) + geom_point() 
