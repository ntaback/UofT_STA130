...
(a) Using the `Galton` data use R to calculate the average and variance of child's height in family's 1, 2, and 32.  Which family has the largest variance?  Explain the meaning of variance in this context.  
...
Family 2 has variance `r summarise(filter(Galton, family == 2), n= n(), mean = mean(height), var = (sd(height))^2)[3]` which is the largest among the three family's. This means that the kids' heights in this family are more *spread out* compared to the other two family's.  Looking at the data makes this clear: two kids' heights are approximately 73in while the other two kids' heights in this family are approximately 66in. 
...
samp <- data_frame(dat)
ssamp_dat <- count(samp, dat)
mutate(ssamp_dat, f = round(n/100000, 3))
```

(d) If the sampling is truly random then what proportion of the sample do expect will contain 0 - 9?  Compare your expectation with the distribution of the numbers 0 - 9 that you found in parts (a) - (c).  Which part is closest to your    
